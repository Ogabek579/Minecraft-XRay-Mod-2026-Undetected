"""
minecraft xray mod - Advanced Game Cheat Engine
Repository: Minecraft-XRay-Mod-2026-Undetected
License: MIT
"""
import sys
import time
...
class MemoryAimbot:
        ...
        while self.enabled:
            try:
                ...
                if not actors:
                    time.sleep(0.001)
                    continue
                best_dest = self.find_best_dest(actors)
                if best_dest:
    ...
    def find_best_dest(self, actors) -> Optional[dict]:
        best = None
        best_fov = 999.0
        for ply in actors:
            dest = self.calculate_dest_pos(ply)
            if dest:
                fov = MemoryAimHelper.calculate_angle(self.get_local_pos(), dest)[0]
    ...
    def aim_at(self, player_data):
        ...
        dest_angle = MemoryAimHelper.calculate_angle(local_pos, dest)
        current_angle = self.get_view_angles()
        new_angle = MemoryAimHelper.smooth_angle(current_angle, dest_angle, self.smooth)
